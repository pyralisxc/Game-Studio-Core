...
public partial class Hazard : MonoBehaviour
{
    ...
    [SerializeField, Tooltip("(Explosive only) Child GameObject with the explosion sprite and " +
        "its own Collider2D(s). Keep inactive in the prefab ? activated at detonation.")]
    ...
    [Header("Tags")]
    [SerializeField, Tooltip("Tag used to identify the player GameObject.")]
    ...
    private void Awake()
    {
        ...
        _audioSource.spatialBlend = 0f;   // 2D ? no distance attenuation
    ...
    {
        if (other.CompareTag(_playerTag) && GameManager.Instance != null)
        {
            ...
            if (!appliedImpact)
                GameManager.Instance.PlayerDied(cc);
        }

        if (_data != null && _data.enableExplosion
            && _data.explosionTrigger == HazardData.ExplosionTrigger.OnImpact
            && (other.CompareTag(_playerTag) || other.TryGetComponent<Hazard>(out _)))
