...
namespace NeonBlack.Gameplay.Features.Hazards
{
    ...
    public class HazardSpawner : MonoBehaviour
    {
        ...
        public void StartSpawning()
        {
            ...
            {
                Debug.LogError("[HazardSpawner] StartSpawning() called with no GameManager in scene. Ensure HazardSpawner only exists in the Game scene.");
                return;
            }
        ...
        private bool TryGetSpawnBounds(float objectRadius, out SpawnBounds bounds)
        {
            ...
            bounds = GetCameraBounds(margin);
            return true;
        }

        private SpawnBounds GetCameraBounds(float margin)
        {
            float baseHalfHeight = CameraAspectController.Instance != null
                ? CameraAspectController.Instance.HalfHeight
                : (_mainCamera != null ? _mainCamera.orthographicSize : 5f);
            float baseHalfWidth = CameraAspectController.Instance != null
                ? CameraAspectController.Instance.HalfWidth
                : (_mainCamera != null ? _mainCamera.orthographicSize * _mainCamera.aspect : 5f);
            Vector3 cameraPosition = _mainCamera != null ? _mainCamera.transform.position : Vector3.zero;

            return new SpawnBounds(
                cameraPosition,
                Mathf.Max(baseHalfWidth - margin, 0.1f),
                Mathf.Max(baseHalfHeight - margin, 0.1f));
        }
        ...
        private void OnDrawGizmos()
        {
            ...
            float margin = _difficultyManager != null ? _difficultyManager.SpawnMargin : 0.5f;
            float halfHeight = Mathf.Max(0.1f, (CameraAspectController.Instance != null ? CameraAspectController.Instance.HalfHeight : cam.orthographicSize) - margin);
            float halfWidth = Mathf.Max(0.1f, (CameraAspectController.Instance != null ? CameraAspectController.Instance.HalfWidth : cam.orthographicSize * cam.aspect) - margin);
            Vector3 center = cam.transform.position;

            Gizmos.color = new Color(0.3f, 0.8f, 1f, 0.25f);
            Gizmos.DrawWireCube(new Vector3(center.x, center.y, 0f), new Vector3(halfWidth * 2f, halfHeight * 2f, 0f));
        }
#endif
    }
}
