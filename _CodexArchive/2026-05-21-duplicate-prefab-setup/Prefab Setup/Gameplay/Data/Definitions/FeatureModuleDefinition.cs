...
namespace NeonBlack.Gameplay.Data.Definitions
{
    ...
    /// </summary>
    [CreateAssetMenu(menuName = "NeonBlack/Gameplay/Definitions/Feature Module Definition", fileName = "FeatureModuleDefinition")]
    public class FeatureModuleDefinition : ScriptableObject
    {
        ...
        public List<string> GetValidationIssues()
        {
            ...
            else
            {
                ...
                AppendRuntimeContractIssues(runtimePrefab, issues);
            }

            switch (moduleId)
            {
                case "actor.interaction":
                    if (!ProfileMatches(profileAsset, typeof(InteractionFeatureProfile)))
                        issues.Add("`actor.interaction` expects an InteractionFeatureProfile profile asset.");
                    break;

                case "actor.traversal.3d":
                    if (profileAsset != null && profileAsset is not PawnTraversalProfile)
                        issues.Add("`actor.traversal.3d` expects a PawnTraversalProfile profile asset.");
                    if (supportedPresentationModes != null && supportedPresentationModes.Length > 0)
                    {
                        for (int i = 0; i < supportedPresentationModes.Length; i++)
                        {
                            if (supportedPresentationModes[i] == ActorPresentationMode.Sprite2D)
                            {
                                issues.Add("`actor.traversal.3d` should not target Sprite2D presentation mode.");
                                break;
                            }
                        }
                    }
                ...
                case "enemy.reaction":
                    if (!ProfileMatches(profileAsset, typeof(EnemyReactionProfile)))
                        issues.Add("`enemy.reaction` expects an EnemyReactionProfile profile asset.");
                    break;

                case "enemy.ambient":
                    if (!ProfileMatches(profileAsset, typeof(EnemyAmbientFeatureProfile)))
                        issues.Add("`enemy.ambient` expects an EnemyAmbientFeatureProfile profile asset.");
                    break;

                case "actor.pickups.2d":
                    if (!ProfileMatches(profileAsset, typeof(PickupFeatureProfile)))
                        issues.Add("`actor.pickups.2d` expects a PickupFeatureProfile profile asset.");
                    if (supportedPresentationModes != null && supportedPresentationModes.Length > 0)
                    {
                        for (int i = 0; i < supportedPresentationModes.Length; i++)
                        {
                            if (supportedPresentationModes[i] != ActorPresentationMode.Sprite2D)
                            {
                                issues.Add("`actor.pickups.2d` currently targets Sprite2D presentation mode only.");
                                break;
                            }
                        }
                    }
                ...
                case "actor.combat.reaction":
                    if (!ProfileMatches(profileAsset, typeof(ActorCombatReactionProfile)))
                        issues.Add("`actor.combat.reaction` expects an ActorCombatReactionProfile profile asset.");
                    break;

                case "actor.status":
                    if (!ProfileMatches(profileAsset, typeof(ActorStatusEffectProfile)))
                        issues.Add("`actor.status` expects an ActorStatusEffectProfile profile asset.");
                    break;

                case "actor.feedback":
                    if (!ProfileMatches(profileAsset, typeof(ActorFeedbackProfile)))
                        issues.Add("`actor.feedback` expects an ActorFeedbackProfile profile asset.");
                    break;

                case "actor.pickups.3d":
                    if (!ProfileMatches(profileAsset, typeof(PickupFeatureProfile)))
                        issues.Add("`actor.pickups.3d` expects a PickupFeatureProfile profile asset.");
                    if (supportedPresentationModes != null && supportedPresentationModes.Length > 0)
                    {
                        for (int i = 0; i < supportedPresentationModes.Length; i++)
                        {
                            if (supportedPresentationModes[i] == ActorPresentationMode.Sprite2D)
                            {
                                issues.Add("`actor.pickups.3d` should target Billboard2_5D and/or Rigged3D presentation modes.");
                                break;
                            }
                        }
                    }
        ...
        private void AppendRuntimeContractIssues(GameObject prefab, List<string> issues)
        {
            switch (moduleId)
            {
                case "actor.interaction":
                    if (!HasComponentImplementing(prefab, "NeonBlack.Gameplay.Features.Interaction.IActorInteractionFeature"))
                        issues.Add("`actor.interaction` runtime prefab should expose IActorInteractionFeature.");
                    break;

                case "actor.traversal.3d":
                    if (!HasComponentImplementing(prefab, "NeonBlack.Gameplay.Features.Traversal.IActorTraversalFeature"))
                        issues.Add("`actor.traversal.3d` runtime prefab should expose IActorTraversalFeature.");
                    break;

                case "actor.pickups.2d":
                case "actor.pickups.3d":
                    if (!HasComponentImplementing(prefab, "NeonBlack.Gameplay.Features.Composition.IActorInteractionHandler"))
                        issues.Add($"`{moduleId}` runtime prefab should expose IActorInteractionHandler.");
                    break;

                case "actor.combat.reaction":
                    if (!HasComponentImplementing(prefab, "NeonBlack.Gameplay.Features.Combat.IActorGuardFeature"))
                        issues.Add("`actor.combat.reaction` runtime prefab should expose IActorGuardFeature.");
                    if (!HasComponentImplementing(prefab, "NeonBlack.Gameplay.Core.Contracts.IDamageModifier"))
                        issues.Add("`actor.combat.reaction` runtime prefab should expose IDamageModifier.");
                    break;

                case "actor.status":
                    if (!HasComponentImplementing(prefab, "NeonBlack.Gameplay.Features.Combat.IActorStatusEffectReceiver"))
                        issues.Add("`actor.status` runtime prefab should expose IActorStatusEffectReceiver.");
                    if (!HasComponentImplementing(prefab, "NeonBlack.Gameplay.Core.Contracts.IDamageModifier"))
                        issues.Add("`actor.status` runtime prefab should expose IDamageModifier for shield-style modifiers.");
                    break;

                case "actor.feedback":
                    if (!HasComponentImplementing(prefab, "NeonBlack.Gameplay.Features.Composition.IActorFeedbackPublisher"))
                        issues.Add("`actor.feedback` runtime prefab should expose IActorFeedbackPublisher.");
                    break;

                case "enemy.reaction":
                    if (!HasComponentImplementing(prefab, "NeonBlack.Gameplay.Features.Enemies.IEnemyReactionState"))
                        issues.Add("`enemy.reaction` runtime prefab should expose IEnemyReactionState.");
                    break;
            }

            AppendRuntimeValidationProviderIssues(prefab, issues);
        }
        ...
        private static bool ProfileMatches(ScriptableObject profile, Type expectedType)
        {
            if (profile == null || expectedType == null)
                return profile == null;

            return expectedType.IsInstanceOfType(profile);
        }
