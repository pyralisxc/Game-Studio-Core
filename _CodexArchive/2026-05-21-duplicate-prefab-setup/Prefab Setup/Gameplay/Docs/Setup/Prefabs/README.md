...
Use these guides after `Docs/Setup/CANONICAL_SETUP.md` and `Docs/Setup/SCENE_SETUP_GUIDE.md`.

Every prefab guide starts with a `Before You Wire This` section. Read that section first, select the matching runtime patterns in a `GameSetupProfile`, assign the profile to `GameModeDefinition.setupProfile`, and resolve validation before wiring scene objects in Unity.
...
Pyralis does not require every game to have a character controller. For chess, cards, checkers, tabletop, puzzle boards, or camera-only interaction:
