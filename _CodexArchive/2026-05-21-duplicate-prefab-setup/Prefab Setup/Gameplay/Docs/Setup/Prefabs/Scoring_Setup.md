...
`CollectibleSpawner2D` integrates with `ParticipantScoreService` automatically — when the player collects an item the service is incremented through the active runtime flow.

To add points manually (e.g. for defeating enemies, events, or bonuses):

```csharp
[Inject] private ParticipantScoreService scoreService;

scoreService?.AddPoints(10);
```

To remove points (penalty mechanics):
...
`UIManager` subscribes to `ParticipantScoreService.OnPointsChanged` automatically and updates the HUD score label. The game-over panel reads the service for final and high-score values.
