...
This means the package now has a real Inspector-driven shared-core startup path, even though major gameplay systems are still in migration.
...
For the 3D brawler, `Motor3D` is the composition root. It coordinates focused sibling components — `Pawn3DInputModule`, `Pawn3DMovementComponent`, Traversal-owned `Pawn3DTraversalComponent`, and `Pawn3DPresentationComponent` — each implementing the corresponding pawn interface directly.

For the 2D stack, `Motor2D` is now only a thin compatibility facade. The actual ownership lives in `PlayerInputHandler`, `PawnCombatBehaviour2D`, `Pawn2DMovementComponent`, and `Pawn2DPresentationComponent`.
...
- the 2D facade remains for package compatibility, but the 2D movement and presentation domains are now separated into dedicated components.
- future cleanup should remove remaining facade-only references where package consumers can be migrated safely.
...
- the package manifest declares Cinemachine as a required dependency; Netcode for GameObjects is an optional extension declared only in `NeonBlack.Gameplay.Networking.asmdef`.
...
- board spaces and legal moves before "chess",
...
- preserve working content while shrinking legacy assumptions,
- retire old paths once migration is proven.
