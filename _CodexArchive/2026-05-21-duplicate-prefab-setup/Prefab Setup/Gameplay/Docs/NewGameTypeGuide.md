...
Use pawns only when the selected runtime patterns need authored actor behavior. Camera-only, board, card, checker, chess, tabletop, puzzle, and turn/menu games can be valid Pyralis games without a character controller.
...
- Board/Card/Tabletop for chess, cards, checkers, tactics boards, resource boards, and piece/zone play
...
For chess, cards, checkers, tabletop, puzzle boards, camera-only viewers, or menu-driven games:
