    ...
    public static class PyralisAuthoringSetupGraphBuilder
    {
            ...
            List<PyralisAuthoringGraphEdge> edges)
        {
            AddNode(nodes, new PyralisAuthoringGraphNode(
                ...
                nativeAction: source is GameplaySessionBootstrap
                    ? null
                    : PyralisAuthoringNativeActionFactory.CreateSceneObjectAction(
                        "Gameplay Root",
                ...
                sourceOrigin: PyralisAuthoringGraphSourceOrigin.Vocabulary,
            ...
            AddNode(nodes, new PyralisAuthoringGraphNode(
                ...
                sourceOrigin: route != null && route.Session != null
                    ...
                    : PyralisAuthoringGraphSourceOrigin.Vocabulary,
            ...
            PyralisReflectedAssignmentState assignmentState)
        {
            ...
            AddNode(nodes, new PyralisAuthoringGraphNode(
                ...
                nativeSetup: new[] { PyralisAuthoringLabelUtility.FormatNativeAction(nativeAction) },
            ...
            List<PyralisAuthoringGraphEdge> edges)
        {
            ...
            AddNode(nodes, new PyralisAuthoringGraphNode(
                ...
                blockingReason: route.HasLocalJoinPolicyConflict()
                    ...
                    : route.HasSoloLocalJoinPolicyGap()
                        ? "Solo local has no PlayerInputManager, so the default participant must auto-register before Play Mode."
                    : string.Empty,
                ...
                issueCode: state == PyralisAuthoringGraphEvidenceState.Missing
                    || state == PyralisAuthoringGraphEvidenceState.Blocked
                        ? route.HasSoloLocalJoinPolicyGap()
                            ? "ParticipantTopology.SoloAutoRegisterMissing"
        ...
        private static string GetParticipantTopologyGuidance(PyralisSetupRouteAnalysis route)
        {
            ...
            {
                return summary + " Local co-op should wait for Unity PlayerInput joins. Disable auto-register defaults so each controller claims one participant seat.";
            }

            if (route.HasSoloLocalJoinPolicyGap())
            {
                return summary + " Solo local has no PlayerInputManager. Enable auto-register defaults so the first participant is present before Play Mode.";
            }
            ...
            {
                return summary + " Solo local routes auto-register the default participant unless a deliberate PlayerInput join path is authored.";
            }
        ...
        private static string[] BuildParticipantTopologyNativeSetup(PyralisSetupRouteAnalysis route)
        {
            ...
            if (route.HasLocalJoinPolicyConflict())
            {
                return new[]
                {
                    "Select ParticipantInputRouter and disable Auto Register Defaults Without Player Input.",
                    "Assign GameplaySessionBootstrap > Player Input Manager so Unity pairs each controller with one joined PlayerInput.",
                    "Keep ParticipantSpawnService > Spawn On Register enabled if pawns should appear when each PlayerInput joins; disable it only for manual spawn flows."
                };
            }

            if (route.HasSoloLocalJoinPolicyGap())
            {
                if (!route.HasParticipantInputRouter)
                {
                    PyralisAuthoringNativeAction action = BuildParticipantInputRouterMissingAction();
                    return new[]
                    {
                        action.ToGuidanceSentence(),
                        "Then enable Auto Register Defaults Without Player Input on ParticipantInputRouter."
                    };
                }

                return new[]
                {
                    "Select ParticipantInputRouter and enable Auto Register Defaults Without Player Input."
                };
            }
        ...
        private static PyralisAuthoringNativeAction? GetParticipantTopologyNativeAction(PyralisSetupRouteAnalysis route)
        {
            ...
            if (route.HasLocalJoinPolicyConflict())
            {
                return new PyralisAuthoringNativeAction(
                    ...
                    "local co-op waits for PlayerInputManager joins");
            }

            if (route.HasSoloLocalJoinPolicyGap())
            {
                if (!route.HasParticipantInputRouter)
                    return BuildParticipantInputRouterMissingAction();
                ...
                return new PyralisAuthoringNativeAction(
                    ...
                    "ParticipantInputRouter",
                    "autoRegisterDefaultParticipantsWithoutPlayerInput",
                    "the solo local participant appears before Play Mode");
            }
        ...
        private static PyralisAuthoringNativeAction BuildParticipantInputRouterMissingAction()
        {
            return PyralisAuthoringNativeActionFactory.AddComponentAction(
                "Game root",
                "ParticipantInputRouter",
                "ParticipantInputRouter appears on the game root, then enable Auto Register Defaults Without Player Input");
        }
            ...
            List<PyralisAuthoringGraphEdge> edges)
        {
            ...
            for (int i = 0; i < snapshot.Rows.Count; i++)
            {
                ...
                AddNode(nodes, new PyralisAuthoringGraphNode(
                    ...
                    blockingReason: row.SupportsProofAttempt ? string.Empty : row.NextFix,
            ...
            List<PyralisAuthoringGraphEdge> edges)
        {
            ...
            AddNode(nodes, new PyralisAuthoringGraphNode(
                ...
                guidance: unresolvedProof
                    ? "No proof is selected yet. Assign a meaningful authored route or use Intent to choose the smallest capability to prove."
                ...
                blockingReason: unresolvedProof ? string.Empty : proofFact != null ? proofFact.Proof : string.Empty,
                sourceOrigin: proofFact != null && proofFact.SourceKind == PyralisAuthoringFactSourceKind.FeatureContract
                    ...
                    : PyralisAuthoringGraphSourceOrigin.VocabularyFallback,
            ...
            if (genericProofTemplate)
            {
                ...
                AddNode(nodes, new PyralisAuthoringGraphNode(
                    ...
                    PyralisAuthoringGraphEvidenceState.Missing,
                    proofTargetId: proofNodeId,
                    guidance: $"{proofFact.DisplayName} is using generic proof vocabulary. Add ProofTargetId to a feature-owned contract when this route needs contract-owned proof meaning.",
                    blockingReason: "The graph can render this proof, but the proof target is still vocabulary-owned rather than contract-owned.",
                    sourceOrigin: PyralisAuthoringGraphSourceOrigin.VocabularyFallback,
            ...
            IReadOnlyList<ResolvedAuthoringContract> sourceContracts)
        {
            ...
            foreach (ResolvedAuthoringContract contract in contractSource)
            {
                ...
                AddNode(nodes, new PyralisAuthoringGraphNode(
                    ...
                    proofTargetId: contract.ProofTargetId,
                ...
                if (!string.IsNullOrWhiteSpace(contract.ProofTargetId)
                    && string.Equals(contract.ProofTargetId, activeProofNodeId, StringComparison.Ordinal))
                {
        ...
        private static void AddSceneReadinessEvidence(UnityEngine.Object source, List<PyralisAuthoringGraphNode> nodes, List<PyralisAuthoringGraphEdge> edges)
        {
            ...
            for (int i = 0; i < issues.Count; i++)
            {
                ...
                PyralisAuthoringNativeAction? nativeAction = BuildSceneReadinessNativeAction(issue);
                AddNode(nodes, new PyralisAuthoringGraphNode(
                    nodeId,
                    GetSceneReadinessEvidenceLabel(issue, nativeAction),
                    PyralisAuthoringGraphNodeKind.ValidationEvidence,
                    PyralisAuthoringGraphSourceKind.SceneReadiness,
                    ConvertSceneReadinessSeverity(issue.Severity),
                    guidance: issue.Message,
                    nativeSetup: nativeAction.HasValue
                        ? new[] { PyralisAuthoringLabelUtility.FormatNativeAction(nativeAction.Value) }
                        : !string.IsNullOrWhiteSpace(issue.NativeAction) ? new[] { issue.NativeAction } : Array.Empty<string>(),
        ...
        private static string GetSceneReadinessEvidenceLabel(
            PyralisSceneReadinessIssue issue,
            PyralisAuthoringNativeAction? nativeAction)
        {
            if (nativeAction.HasValue)
                return PyralisAuthoringLabelUtility.GetNativeActionDisplayLabel(nativeAction.Value);

            return issue != null ? issue.Category.ToString() : "Scene Readiness";
        }

        private static PyralisAuthoringNativeAction? BuildSceneReadinessNativeAction(PyralisSceneReadinessIssue issue)
        {
            if (issue == null
                || string.IsNullOrWhiteSpace(issue.NativeActionTarget)
                || string.IsNullOrWhiteSpace(issue.NativeActionFieldOrComponent))
            {
                return null;
            }

            PyralisAuthoringNativeAction action = PyralisAuthoringNativeActionFactory.AddComponentAction(
                issue.NativeActionTarget,
                issue.NativeActionFieldOrComponent,
                string.Empty);
            string successCheck = !string.IsNullOrWhiteSpace(issue.NativeAction)
                ? issue.NativeAction
                : PyralisAuthoringLabelUtility.GetNativeActionOwnerLabel(action) + " is ready before Play Mode.";
            return PyralisAuthoringNativeActionFactory.AddComponentAction(
                issue.NativeActionTarget,
                issue.NativeActionFieldOrComponent,
                successCheck);
