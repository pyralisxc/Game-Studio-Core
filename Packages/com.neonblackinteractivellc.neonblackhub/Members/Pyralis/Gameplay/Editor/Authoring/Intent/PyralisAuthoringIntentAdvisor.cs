...
namespace NeonBlack.Gameplay.Editor
{
    ...
    public static class PyralisAuthoringIntentAdvisor
    {
        ...
        {
            return Build(selection, PyralisAuthoringVocabularyRegistry.AllFacts);
        }
        ...
        private static string BuildProofFocusLabel(PyralisAuthoringIntentSelection selection)
        {
            ...
            PyralisAuthoringFact proof = PyralisAuthoringVocabularyRegistry.AllFacts.FirstOrDefault(fact =>
        ...
        private static string BuildProofFocusDetail(PyralisAuthoringIntentSelection selection)
        {
            ...
            if (string.Equals(proofId, "proof.1p-pawn-movement", StringComparison.Ordinal))
                return "Target route focus is one responsive pawn. If another selected ingredient declares a narrower proof target, that focused proof should replace pawn movement while keeping pawn setup as prerequisite evidence.";
