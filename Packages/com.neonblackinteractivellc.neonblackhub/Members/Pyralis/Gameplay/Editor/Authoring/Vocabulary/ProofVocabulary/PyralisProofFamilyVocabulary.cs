    ...
    public static class PyralisProofFamilyVocabulary
    {
        ...
        public static System.Collections.Generic.IReadOnlyList<PyralisAuthoringFact> GetDefaultProofTemplates()
        {
            ...
            facts.Add(CreateGenericProofFact(
                ...
                "Run one local pawn-backed movement proof when the selected Intent focus is responsive pawn control or when no narrower feature proof has been selected yet.",
                ...
                "one pawn spawns at the assigned spawn point, visibly moves through its selected 2D, 2.5D, or 3D lane, and the Game view follows the runtime GameplaySharedCameraFocus driven by that pawn",
                "The 1P pawn proof is the focused checkpoint for responsive pawn control. Use it as fallback or prerequisite evidence, not as a replacement for a selected feature proof.",
                string.Empty,
                new[]
                {
                    "route.pawn-actor",
            ...
            facts.Add(CreateGenericProofFact(
                ...
                "Run one local multiplayer pawn proof when the selected Intent focus is local join or when a focused proof needs multiple local participants.",
                ...
                "The local co-op pawn proof verifies input isolation, join policy, participant seating, spawn placement, and camera focus for routes that select or depend on local multiplayer.",
                string.Empty,
                new[]
                {
                    "route.pawn-actor",
