...
namespace NeonBlack.Gameplay.Editor
{
    internal static class PyralisAuthoringTabRenderer
    {
        public static VisualElement BuildOverview(PyralisAuthoringOverviewProjection projection, System.Action openIntent, System.Action openGuide, System.Action openMap)
        {
            ...
            string guidance = projection.CurrentStep != null && !string.IsNullOrWhiteSpace(projection.CurrentStep.Message) ? projection.CurrentStep.Message : projection.Model.ProofGuidance;
            ...
            DrawOverviewLane(page, "Proof Enhancers", "Useful before Play Mode when they make the selected proof clearer.", projection.Model.DoSoon, 3);
            DrawProof(page, projection);
            DrawSelectedIntentFollowThrough(page, projection.SelectedIntentStatus, compact: true);
            return page;
        }
        ...
        public static VisualElement BuildGuide(PyralisAuthoringGuideProjection projection)
        {
            ...
            page.Add(PyralisAuthoringUi.ActionRow(PyralisAuthoringGraphJsonExportControl.BuildGuideSnapshotButton(projection?.GuideTrace)));
            if (projection == null)
                return page;
            if (projection.SelectionFirst)
                DrawSelectedContext(page, projection);
            DrawRouteChecklist(page, projection);
            DrawSelectedIntentFollowThrough(page, projection.SelectedIntentStatus, compact: false);
            DrawReflectiveContracts(page, projection.Contracts);
            if (!projection.SelectionFirst)
                DrawSelectedContext(page, projection);
            DrawGuideTrace(page, projection);
            return page;
        }
        ...
        private static void DrawProof(VisualElement parent, PyralisAuthoringOverviewProjection projection)
        {
            VisualElement section = PyralisAuthoringUi.Section(parent, "Proof After Do Now");
            PyralisAuthoringOverviewModel model = projection.Model;
            PyralisAuthoringGraphNode proofNode = projection.ProofNode;
            PyralisAuthoringUi.Field(section, "Proof", proofNode != null ? proofNode.Label : model.ProofLabel);
            PyralisAuthoringUi.Field(section, "When To Test", GetFlowTestStatus(model));
            PyralisAuthoringUi.Field(section, "Play Mode Action", GetFirstValue(proofNode?.NativeSetup, model.ProofSetupSurface));
            PyralisAuthoringUi.Field(section, "Success Looks Like", !string.IsNullOrWhiteSpace(proofNode?.BlockingReason) ? proofNode.BlockingReason : model.ProofSuccessCriteria);
            PyralisAuthoringUi.Field(section, "Do Later", model.ProofDeferUntilAfter);
        }
        ...
        private static string GetFlowTestStatus(PyralisAuthoringOverviewModel model)
        {
            ...
            if (model.DoNow.Count > 0)
                return "Not ready to test yet. Clear Do Now in Edit Mode first, then use Play Mode only for the proof test.";
            if (model.DoSoon.Count > 0)
                return "Ready for a Play Mode proof. Proof Enhancers can make the test clearer, but setup edits still belong in Edit Mode.";
            return "Ready for proof. Run the route pass named below, verify the selected interaction in Play Mode, stop Play Mode, then add one feature at a time.";
        }
        ...
        private static void DrawSelectedIntentFollowThrough(
            VisualElement parent,
            IReadOnlyList<PyralisAuthoringSelectedIntentStatusRow> rows,
            bool compact)
        {
            VisualElement section = PyralisAuthoringUi.Section(
                parent,
                "Selected Intent Follow-through",
                "These selected ingredients show what is in the proof path, what supports it, and what can wait until after this proof.");
            int count = rows?.Count ?? 0;
            PyralisAuthoringUi.Field(section, "Selected Ingredients", count.ToString());
            if (count == 0)
            {
                PyralisAuthoringUi.Mini(section, "No selected gameplay ingredients are projected yet.");
                return;
            }

            int proofPathCount = CountSelectedIntentStatus(rows, PyralisAuthoringSelectedIntentStatus.InProofPath)
                + CountSelectedIntentStatus(rows, PyralisAuthoringSelectedIntentStatus.SupportsProof);
            int setupCount = CountSelectedIntentStatus(rows, PyralisAuthoringSelectedIntentStatus.NeedsSetupBeforeProof)
                + CountSelectedIntentStatus(rows, PyralisAuthoringSelectedIntentStatus.SupportsProofNeedsSetup);
            int followThroughCount = CountSelectedIntentStatus(rows, PyralisAuthoringSelectedIntentStatus.NextAfterProof)
                + CountSelectedIntentStatus(rows, PyralisAuthoringSelectedIntentStatus.ProofEnhancer);
            int missingCount = CountSelectedIntentStatus(rows, PyralisAuthoringSelectedIntentStatus.MissingGraphEvidence);
            PyralisAuthoringUi.Field(section, "Proof Path / Setup / Follow-through / Missing", $"{proofPathCount} / {setupCount} / {followThroughCount} / {missingCount}");

            int visible = compact ? System.Math.Min(count, 5) : count;
            for (int i = 0; i < visible; i++)
                DrawSelectedIntentStatus(section, rows[i]);
            if (count > visible)
                PyralisAuthoringUi.Mini(section, $"{count - visible} more selected ingredient(s) are visible in Guide.");
        }

        private static int CountSelectedIntentStatus(
            IReadOnlyList<PyralisAuthoringSelectedIntentStatusRow> rows,
        ...
        private static void DrawSelectedIntentStatus(
            VisualElement parent,
            PyralisAuthoringSelectedIntentStatusRow row)
        {
            if (row == null)
                return;

            VisualElement card = PyralisAuthoringUi.Card(parent, row.Label, FormatSelectedIntentStatus(row.Status));
            PyralisAuthoringUi.Field(card, "Group", string.IsNullOrWhiteSpace(row.Subgroup) ? row.Group : row.Group + " / " + row.Subgroup);
            PyralisAuthoringUi.Field(card, "Route Relationship", row.RouteEvidence);
            PyralisAuthoringUi.Field(card, "Evidence", PyralisAuthoringUi.Evidence(row.EvidenceState));
            PyralisAuthoringUi.Field(card, "Next Surface", row.NextSurface);
            PyralisAuthoringUi.Mini(card, row.Summary);
        }

        private static string FormatSelectedIntentStatus(PyralisAuthoringSelectedIntentStatus status)
        {
            return status switch
            {
                PyralisAuthoringSelectedIntentStatus.InProofPath => "in proof path",
                PyralisAuthoringSelectedIntentStatus.SupportsProof => "supports proof",
                PyralisAuthoringSelectedIntentStatus.NeedsSetupBeforeProof => "needs setup before proof",
                PyralisAuthoringSelectedIntentStatus.SupportsProofNeedsSetup => "supports proof; needs setup evidence",
                PyralisAuthoringSelectedIntentStatus.ProofEnhancer => "proof enhancer",
                PyralisAuthoringSelectedIntentStatus.NextAfterProof => "next after proof",
                _ => "missing graph evidence"
            };
        }
        ...
        private static void DrawGuideTrace(VisualElement parent, PyralisAuthoringGuideProjection projection)
        {
            PyralisAuthoringGuideTraceProjection trace = projection?.GuideTrace;
            PyralisAuthoringRouteWorkingProjection route = trace?.Route;
            Foldout foldout = PyralisAuthoringUi.Foldout(parent, "Guide Trace", false);
            PyralisAuthoringUi.Help(foldout, "Debug view of the same Guide projection exported by this tab. It explains why this proof path is active without becoming a separate proof engine.");
            PyralisAuthoringUi.Field(foldout, "Route", trace?.Graph != null ? trace.Graph.RouteName : "No setup route selected");
            PyralisAuthoringUi.Field(foldout, "Intent Focus", PyralisAuthoringSetupGraphProjection.BuildIntentFocusSummary(trace?.Graph));
            PyralisAuthoringUi.Field(foldout, "Route Shape", PyralisAuthoringSetupGraphProjection.BuildRouteShapeSummary(trace?.Graph));
            PyralisAuthoringUi.Field(foldout, "Proof Priority", PyralisAuthoringSetupGraphProjection.BuildProofPrioritySummary(trace?.Graph));
            PyralisAuthoringUi.Field(foldout, "Proof", route?.Proof != null ? route.Proof.Label : "No proof target projected");
            PyralisAuthoringUi.Field(foldout, "Current Action", route?.CurrentAction != null ? route.CurrentAction.Label : "No current action");
            PyralisAuthoringUi.Field(foldout, "Ready For Proof", route != null && route.ReadyForProof ? "Yes" : "No");
            PyralisAuthoringUi.Field(foldout, "Ordered / Critical / Enhancers / Blockers", route != null
                ? $"{route.OrderedSteps.Count} / {route.CriticalPath.Count} / {route.ProofEnhancers.Count} / {route.ProofBlockers.Count}"
                : "0 / 0 / 0 / 0");
            DrawGuideTraceRows(foldout, "Critical Path", route?.CriticalPath, 8);
            DrawGuideTraceConnections(foldout, "Proof Blockers", route?.ProofBlockers, 8);
            DrawGuideTraceConnections(foldout, "Proof Support", route?.ProofSupport, 8);
            DrawGuideTraceContracts(foldout, trace?.SupportingContracts, 8);
            DrawGuideTraceQuestions(foldout, trace?.DiagnosticQuestions);
        }

        private static void DrawGuideTraceRows(VisualElement parent, string title, IReadOnlyList<PyralisAuthoringRouteStepRow> rows, int visibleLimit)
        {
            int count = rows?.Count ?? 0;
            Foldout foldout = PyralisAuthoringUi.Foldout(parent, $"{title} ({count})", false);
            int visible = System.Math.Min(count, visibleLimit);
            for (int i = 0; i < visible; i++)
            {
                PyralisAuthoringRouteStepRow row = rows[i];
                PyralisAuthoringUi.Field(foldout, row.Label, $"{row.PhaseLabel} / {PyralisAuthoringUi.Evidence(row.EvidenceState)}");
            }

            if (count > visible)
                PyralisAuthoringUi.Mini(foldout, $"{count - visible} more row(s) are included in the Guide JSON export.");
        }

        private static void DrawGuideTraceConnections(VisualElement parent, string title, IReadOnlyList<PyralisAuthoringGraphConnectionRow> rows, int visibleLimit)
        {
            int count = rows?.Count ?? 0;
            Foldout foldout = PyralisAuthoringUi.Foldout(parent, $"{title} ({count})", false);
            int visible = System.Math.Min(count, visibleLimit);
            for (int i = 0; i < visible; i++)
            {
                ...
                PyralisAuthoringUi.Field(foldout, row.ToLabel, row.FromLabel);
            }

            if (count > visible)
                PyralisAuthoringUi.Mini(foldout, $"{count - visible} more connection(s) are included in the Guide JSON export.");
        }

        private static void DrawGuideTraceContracts(VisualElement parent, IReadOnlyList<PyralisAuthoringGraphNode> contracts, int visibleLimit)
        {
            int count = contracts?.Count ?? 0;
            Foldout foldout = PyralisAuthoringUi.Foldout(parent, $"Supporting Contracts ({count})", false);
            int visible = System.Math.Min(count, visibleLimit);
            for (int i = 0; i < visible; i++)
            {
                PyralisAuthoringGraphNode node = contracts[i];
                PyralisAuthoringUi.Field(foldout, node.Label, string.IsNullOrWhiteSpace(node.ProofTargetId) ? node.StableId : node.ProofTargetId);
            }

            if (count > visible)
                PyralisAuthoringUi.Mini(foldout, $"{count - visible} more contract(s) are included in the Guide JSON export.");
        }

        private static void DrawGuideTraceQuestions(VisualElement parent, IReadOnlyList<PyralisAuthoringRouteDiagnosticQuestionRow> rows)
        {
            int count = rows?.Count ?? 0;
            Foldout foldout = PyralisAuthoringUi.Foldout(parent, $"Diagnostic Questions ({count})", false);
            for (int i = 0; i < count; i++)
            {
                PyralisAuthoringRouteDiagnosticQuestionRow row = rows[i];
                PyralisAuthoringUi.Field(foldout, row.Question, row.Answer);
            }
        }
        ...
        private static void DrawContract(VisualElement parent, ResolvedAuthoringContract contract)
        {
            ...
            PyralisAuthoringUi.Field(card, "Proof Target", string.IsNullOrWhiteSpace(contract.ProofTargetId) ? "None recorded yet." : contract.ProofTargetId);
        }
        ...
        private static void DrawFact(VisualElement parent, PyralisAuthoringFact fact)
        {
            ...
            PyralisAuthoringUi.Field(card, "Proof", fact.Proof);
        }
        ...
        private static void DrawNativeAction(VisualElement parent, PyralisAuthoringNativeAction action)
        {
            ...
            PyralisAuthoringUi.Field(card, "Field / Component", PyralisAuthoringLabelUtility.GetFieldOrComponentLabel(action.FieldOrComponent));
