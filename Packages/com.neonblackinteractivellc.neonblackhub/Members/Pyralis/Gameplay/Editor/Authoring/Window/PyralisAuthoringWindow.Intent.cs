    ...
    public partial class PyralisAuthoringWindow
    {
        ...
        private void RefreshIntentTab()
        {
            ...
            var exportButton = new Button(() =>
            {
                ...
                Object activeSetup = ResolveCurrentActiveSetup(Selection.activeObject);
                PyralisAuthoringSetupGraph graph = GetCachedIntentProjectedSetupGraph(activeSetup != null ? activeSetup : Selection.activeObject);
            ...
            exportButton.tooltip = "Write the Intent projection snapshot: DNA axioms, presentation lane, participant route, capability descriptors, metadata backlog, selected ingredients, route-shape summary, and advisor rows. It does not export scene/setup reality or proof/setup status.";
        ...
        private void UpdateAdvisor(VisualElement root)
        {
            ...
            if (proofLabel != null)
            {
                Object activeSetup = ResolveCurrentActiveSetup(Selection.activeObject);
                PyralisAuthoringSetupGraph graph = GetCachedIntentProjectedSetupGraph(activeSetup != null ? activeSetup : Selection.activeObject);
                proofLabel.text = BuildIntentProofFocusSummary(model, graph);
            }
            ...
            PyralisAuthoringSetupGraph graph)
        {
            ...
            if (proof == null || string.IsNullOrWhiteSpace(proof.Label))
                return model != null ? model.ProofFocusSummary : string.Empty;

            string detail = !string.IsNullOrWhiteSpace(proof.Guidance)
                ? proof.Guidance
                : proof.BlockingReason;
            return string.IsNullOrWhiteSpace(detail)
                ? "Target proof: " + proof.Label
                : "Target proof: " + proof.Label + ". " + detail;
        }
