...
The goal is not to make every genre a separate framework. The platform goal is to identify reusable capability families and compose them through definitions, profiles, feature modules, editor tooling, and samples that work for all supported routes, not just one lane.

## Route Completeness

The product unit is a complete route, not a single mechanic.

A route is complete when Pyralis provides:

- mechanic runtime
- definitions and profiles
- feature-owned authoring contracts when the route uses reusable feature modules
...
This is the difference between a pile of systems and a game studio in a box. New runtime work should be paired with its authoring path as soon as possible, and authoring work should point toward the smallest playable proof instead of asking the developer to wire every optional system at once.
...
The long-term feature map includes authoring and learning tools, core game loop systems, combat/action systems, weapons/projectiles, enemy/AI, brawler routes, arena shooter routes, survival swarm routes, board/tactics routes, card/deckbuilder routes, RPG/progression, UI/HUD/menus, camera/presentation, procedural/content generation, multiplayer/networking, Steam/shipping, and studio production tools. These should be sequenced through route completeness, with the Authoring Window first because it makes every route understandable.
