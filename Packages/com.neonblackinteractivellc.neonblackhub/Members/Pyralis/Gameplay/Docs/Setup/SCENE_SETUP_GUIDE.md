...
Start with `Docs/Setup/START_HERE.md` and run the
`authoring-native-1p-proof-checklist.md` pass from the repository `docs/` root first.
Read `Docs/Setup/AUTHORING_MODEL.md` if definitions and profiles are unclear,
then use `Docs/Setup/Prefabs/Bootstrap_Example_Setup.md` for click-by-click Editor wiring.
Use `Docs/Setup/MANUAL.md` only when you want the book-style index.
