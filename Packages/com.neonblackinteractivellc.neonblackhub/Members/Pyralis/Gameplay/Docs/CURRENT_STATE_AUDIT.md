...
- compatibility singleton owners now clean up after themselves during teardown: `TimeManager`, `SettingsManager`, and `GameManager` clear their static surfaces on destruction or subsystem registration, and `PlayerSpawner` destroys its generated `DontDestroyOnLoad` countdown UI instead of leaking persistent canvases across scene/test teardown
