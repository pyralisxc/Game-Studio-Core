...
- service lookup migration support is narrower: active feature code should use bootstrap-owned services, VContainer injection, explicit runtime configuration, or participant/session references instead of adding a second service-location layer
...
- setup flow now has an explicit `Runtime Service Ownership` row so Unity users see that `GameplaySessionBootstrap` configures `PyralisGameplayLifetimeScope`; remaining static `Instance` and participant-query surfaces are compatibility affordances, not the path for new service dependencies
...
- session state ownership is centralized: `SessionStateService` is the normal `IGameplayStateReader`, while the 2D `GameManager` owns arcade flow through `IGameplaySessionFlow` and pushes Playing/Results phase changes into the session state instead of becoming a second gameplay-state owner.
- Hygiene ownership is explicit: source and asset hygiene scanners live under `Editor/Authoring/Spine/Hygiene`, readiness projections use graph audit rows, and `PyralisGameplayLifetimeScope` labels loaded-scene service scans as compatibility evidence rather than primary route truth.
- compatibility singleton owners now clean up after themselves during teardown: `TimeManager` and `SettingsManager` clear their static surfaces on destruction or subsystem registration, and `PlayerSpawner` destroys its generated `DontDestroyOnLoad` countdown UI instead of leaking persistent canvases across scene/test teardown
...
- the latest hygiene-driven source-lane pass keeps beginner-facing runtime components stable while making their internal ownership harder to blur: 2D movement config/validation/gizmos, 3D motor frame loop, 3D traversal profiles/references, enemy combat execution, enemy reaction feature wiring, EnemyAI feature/gizmo support, shared damage-zone target ticking, world health bar runtime/events, floating-feedback popups/validation, and 2D presentation animation/sprite/deformation lanes now live in focused partials.
