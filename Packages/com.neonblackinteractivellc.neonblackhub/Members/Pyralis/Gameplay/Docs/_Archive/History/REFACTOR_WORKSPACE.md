...
- `Motor3D` - 3D pawn coordinator; implements `ICharacterMotorState`
- `Pawn3DInputModule` - all Input System binding; produces `FrameInput` per frame
- `Pawn3DMovementComponent` - `BrawlerMovementModel` + `CharacterController`; implements `IPawnMotor`
- `Pawn3DTraversalComponent` - climb, hang, shimmy, ledge detection; implements `IPawnTraversalModule`
- `Pawn3DPresentationComponent` - Animator, billboard, land squash, debug HUD; implements `IPawnPresentationModule`
- `FrameInput` - per-frame input snapshot struct; produced by `Pawn3DInputModule`, consumed by all modules
- `Pawn2DMovementComponent` - 2D movement, dash, bounds, and reaction-lock ownership
- `Pawn2DPresentationComponent` - 2D presentation ownership
- `CockroachController.ApplyMovementProfile`, `ApplyPresentationProfile` - public profile API added
...
- pawn starter pack generator in the Editor
...
- `SettingsMenu` settings duplication resolved - now delegates all volume changes to `SettingsManager`; duplicate AudioMixer and `Vol_*` PlayerPrefs keys removed,
...
- `PlayerActions` (1,199 lines) removed - replaced by `Motor3D` (coordinator) + `Pawn3DInputModule`, `Pawn3DMovementComponent`, `Pawn3DTraversalComponent`, `Pawn3DPresentationComponent`; `FrameInput` struct introduced,
...
- `Data/` - definitions and profiles
- `Editor/` - flat editor tooling
- `Features/Characters/` (+ `2D/`, `3D/`) - pawns, participants, bridges
...
- `Networking/` (+ `Characters/`) - NGO extension layer; package-level dependencies are present for compilation, but gameplay use stays opt-in through a separate assembly with no core reverse-dependency
- `Docs/` - internal notes
...
- `Assets/Create/NeonBlack/Pawn Starter Pack` generates a starter profile/definition asset set.
