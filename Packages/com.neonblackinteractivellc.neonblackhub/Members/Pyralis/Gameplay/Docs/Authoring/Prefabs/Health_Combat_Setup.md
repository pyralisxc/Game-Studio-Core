...
- **Hit Pause Sink** - assign `TimeManager` or another component that implements `IHitPauseSink` when freeze frames are enabled.

**Camera Shake** (optional)
- **Camera Shake Intensity** - `0.15` for a punch, `0.3` for a heavy hit. `0` to disable.
- **Camera Shake Sink** - assign `CameraShake` or another component that implements `ICameraShakeSink` when shake is enabled.
...
Hit pause and camera shake are explicit service hooks. New prefabs should wire the sinks in the Inspector or have the spawn/bootstrap path call `SetHitPauseSink`, `SetCameraShakeSink`, or `SetImpactFeedbackSinks`.
...
1. Add `DamageNumberSpawner` to a persistent gameplay systems object.
2. Assign **Popup Camera** to the gameplay camera that should face the floating numbers.
3. Set **Initial Pool Size** high enough for the busiest expected burst of simultaneous hits.
4. On `WorldHealthBar` or `ActorFloatingFeedbackReceiver`, assign **Damage Number Sink** to this `DamageNumberSpawner` when damage/heal number categories are enabled.

`DamageNumberSpawner` implements `IDamageNumberSink`, so custom combat feedback services can replace it without changing health bar or feedback receiver code.
