...
Current implementation: selected setup assets project as `UserAuthoredSetup` graph nodes. Missing setup concepts remain `SpineGrammar`. Explicit contracts project as `Contract`; inferred or convention-derived contracts project as `Reflection`. This provenance is audit metadata, not a new authoring source.
...
- `PyralisSetupRouteAnalysis` interprets reflected route capabilities from authored setup, contracts/reflection, scene evidence, and grammar vocabulary.
...
Intent remains the route-filtering surface. It records explicit creator-facing filters such as world/playfield, control shape, presentation lane, capability toggles, and proof goals for the Authoring Window. The graph explains the consequences of those choices without mutating gameplay assets.
...
- selected `GameplaySessionBootstrap`, `SessionDefinition`, `GameModeDefinition`, or `SessionDefinition`/`GameModeDefinition` route
...
Current implementation: scene-readiness issues are reflected as deterministic `ValidationEvidence` graph nodes, and readiness surfaces render required, recommended, and proof-enhancer buckets from `PyralisAuthoringSetupGraphProjection.BuildReadinessAuditRows`.
...
Current implementation: the Authoring Window no longer caches or passes `PyralisAuthoringRouteReport` into visible tabs. Readiness buckets and detailed evidence cards are graph-backed through `PyralisAuthoringSetupGraphProjection.BuildReadinessAuditRows`. The older `PyralisAuthoringValidationModel` card surface was removed after typed issue projection moved to `PyralisAuthoringSetupGraphProjection.BuildTypedValidationIssues`; do not reintroduce tab-local validation card models.
Current implementation: the visible Hygiene tab reads `PyralisAuthoringSetupGraph` through graph hygiene projections. Concrete setup-flow and scene-readiness validators remain graph inputs inside the builder; Hygiene must not call those validators or legacy route/validation models directly.
...
Current implementation: selected setup assets, bootstrap roots, pawn roots, reflected contracts, and route capability context resolve through `PyralisAuthoringSetupGraphProjection.BuildSelectedContextRow`, and Guide uses that row for role, graph id, evidence, native setup, and first check. Keep specialized selection renderers only where they expose object-specific actions such as selecting scene components or pinging targets.
...
Current implementation: Guide path rows use `PyralisAuthoringSetupGraphProjection.BuildRouteStepRows(...)` only when the graph has a resolved setup context such as a Bootstrap, SessionDefinition, GameModeDefinition, ParticipantDefinition, PawnDefinition, or Gameplay Root. Those rows are generated from first unresolved graph nodes, active proof nodes, proof support/blocker edges, selected capability nodes, and reflected contracts, then labeled with derived route phases such as Foundation, Setup Chain, Capability, Scene Evidence, First Proof, Validation, and Can Wait. `PyralisAuthoringIntentAdvisor` remains the pre-setup/fallback planning model; do not use it as the primary setup-backed Guide source.
...
Current implementation: Overview surfaces the active Intent focus and first-proof priority explicitly. Map does not surface Intent focus; it shows current scene/setup reality, setup chain rows, scene-surface evidence, missing field/component/setup issues, and collapsed developer route connections. Hygiene owns programmer-facing graph and code health: proof blocker links, stable graph ids, evidence origins, contract/source pressure, dependency pressure, and deeper graph reasoning. Concrete Unity repair rows stay in Map even when the same scene-readiness evidence exists inside the graph. Plain `Input` intent contributes to selection/action targeting; it does not infer camera/framing unless `Camera`, camera lane evidence, or a visual proof requirement asks for it.
...
Current implementation: proof nodes derive their evidence state from graph support and blocker edges after setup-flow and scene-readiness evidence has been compiled. A proof with blocked prerequisites is `Blocked`, a proof with missing required prerequisites is `Missing`, and a proof with supporting capability edges and no required blockers is `CandidateDetected`. Proof vocabulary can still name the fallback proof and provide beginner wording, but proof readiness comes from graph evidence.
Current implementation: feature-module route capabilities come from resolved authoring contracts and reflected descriptors, not from module id, display name, or authoring category keyword guesses. Uncontracted modules should stay semantically quiet until the feature owner declares contract metadata or reflection can prove the capability.
...
- user-authored setup: selected session/mode assets, participant/pawn definitions, feature-module choices, scene objects, and route declarations
...
- a `SessionDefinition` and `GameModeDefinition` with pawn/combat participants, feature modules, or contracts build setup, capability, and proof nodes
...
- grammar vocabulary and proof vocabulary project as graph nodes instead of staying hidden in route analysis
