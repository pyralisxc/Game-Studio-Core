...
- `PyralisAuthoringRouteProof` remains route-derived proof identity and proof-chain composition; `PyralisProofFamilyVocabulary` owns fallback proof templates; `PyralisAuthoringOverviewModel`, graph projections, and tab renderers project resolved setup graph output.
...
- route-derived proof identity from `PyralisAuthoringRouteProof`
