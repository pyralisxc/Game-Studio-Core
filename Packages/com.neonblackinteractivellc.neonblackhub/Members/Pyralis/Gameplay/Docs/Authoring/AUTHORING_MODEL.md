...
- `Camera Rig Controller`: connects the bootstrap to the Pyralis Cinemachine camera flow. It owns view framing and visible camera bounds. `PlayfieldProfile` owns legal movement bounds, clamp, wrap, and arena depth. Sprite2D movement may use camera-visible bounds as a fallback when no playfield bounds are authored; Billboard2_5D and Rigged3D pawn patterns still need a camera rig for framing, but should not be blocked on orthographic 2D bounds.
