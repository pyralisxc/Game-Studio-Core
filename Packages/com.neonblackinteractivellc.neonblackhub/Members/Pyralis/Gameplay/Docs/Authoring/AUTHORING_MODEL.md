...
Map and Hygiene can each export a **Graph JSON Snapshot** from a compact tab-local button. This is an editor-only, read-only diagnostic packet for humans, agents, and issue reports. Each export writes the current tab's graph view into `Editor/Authoring/TempGraphs`, serializing the resolved setup graph, stable node ids, edges, evidence states, source origins, native actions, Map rows, and Hygiene rows. It is not gameplay data, not a preset, not a generated fix plan, and not a second source of setup truth. Generated snapshots in that folder are temp diagnostics and should stay ignored.
