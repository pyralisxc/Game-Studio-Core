...
|---|---|---|---|
| `SessionDefinition` | session name, max participants, join/camera defaults | default `GameModeDefinition`, shared fallback `InputProfile`, `SettingsProfile`, participant list | every playable setup |
...
- **Validate**: the graph integrity console; shows blockers, recommended edits, proof enhancers, source origins, stable graph ids, and proof reachability for the selected setup object and its inferred route.
...
The **Intent**, **Guide**, **Overview**, **Map**, **Validate**, and **Facts** tabs are projections from the same authoring spine, but they do not all use the same lens. The **current setup graph** is built from contracts, dependency-tree route analysis, setup-flow evidence, scene-readiness evidence, selected Unity context, and generic grammar fallback wording. **Map**, **Validate**, and **Facts** use that current setup graph so they show what Unity and authored assets actually prove. **Map** presents the concrete scene/setup consequences of that graph: missing scene surfaces, empty fields, component requirements, selected-object wiring, and current topology. **Validate** presents graph integrity: blocker buckets, evidence states, source origins, stable ids, proof reachability, and source detail without becoming a second scene walkthrough. The **intent-projected graph** starts from the current setup graph and applies the active Intent selection as a focus lens for first-proof guidance. **Overview** and **Guide** use that intent projection so they can tell the creator which setup tissue to connect next without pretending the route is already authored.
