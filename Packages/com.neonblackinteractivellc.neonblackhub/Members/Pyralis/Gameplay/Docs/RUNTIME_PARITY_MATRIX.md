...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Action and targeting | `ActionDefinition` | `ActionExecutionContext`, target descriptors, validation and resolution results, `QueuedAction` | `IActionResolver`, `IActionQueueService`, `ActionQueueService` | Definition validation | Runtime and editor tests | Pawn starter pack projectile assets use this foundation | Guided Needs Proof |
| Guns and projectiles | `ProjectileDefinition`, `FireModeDefinition`, `ProjectileImpactDefinition` | Fire requests, spawn commands, magazine state | 2D and 3D launchers, pooling handle, impact effect player | Definition validation | Runtime and editor tests | Starter pack projectile/fire-mode/impact assets | Guided Needs Proof |
| Runtime pattern setup | `RuntimePatternDefinition`, `GameSetupProfile` | Pattern/profile validation data | Game mode setup-profile validation | Custom inspectors and validation tests | Editor tests | Starter pack runtime pattern assets | Guided Needs Proof |
| Core Rules Spine | `BoardDefinition`, `BoardPieceDefinition`, `BoardMovePolicyDefinition`, `BoardTerminalConditionDefinition`, `PhaseDefinition`, `TurnOrderDefinition`, `GameModeDefinition` links | `BoardRuntimeState`, `BoardSpaceState`, `BoardPieceState`, `BoardCoordinate`, `TurnRuntimeState`, `BoardMoveActionPayload`, `BoardMovePolicy`, `BoardTerminalCondition` | `IBoardStateService`, `IBoardMovePolicy`, `IBoardTerminalCondition`, `ITurnOrderService`, `BoardMoveActionResolver`, `TurnAdvanceActionResolver` | Board, piece, move-policy, terminal-condition, phase, turn, and game-mode validation | Runtime and editor tests | Tabletop starter pack uses this spine | Guided Needs Proof |
| Board/card/tabletop | Board, move-policy, terminal-condition, and turn starter assets | Board occupancy, movement, capture state, shape and offset move policy evaluation, terminal-state evaluation, turn cursor | Board move and turn-advance action resolvers | Board, move-policy, terminal-condition, turn, and setup-flow claim validation | Foundation tests and starter-pack source contracts | Tabletop starter pack; scene/UI proof still pending | Guided Needs Proof |
...
| 3D brawler | Existing pawn, combat, traversal, enemy, zone, and camera definitions/profiles | 3D pawn, combat, traversal, enemy, and encounter runtime | Explicit runtime modules | Existing custom inspectors | Existing EditMode and PlayMode coverage | Pawn starter pack and existing scenes | Guided Needs Proof |
...
5. Keep Unity-only authoring UX current with guided inspectors, Create Asset menu coverage, starter packs, and setup validation for each creator-facing asset.
