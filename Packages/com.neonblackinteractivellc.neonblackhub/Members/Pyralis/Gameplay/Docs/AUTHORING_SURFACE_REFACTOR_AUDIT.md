...
- Focused wiring-report runtime tests now cover the first canonical-evidence parity behaviors: missing `SessionDefinition` setup appears once as a `MissingProvider` row; route-dependent camera guidance is deferred while session route evidence is absent; participant service and feature activation rows wait for a complete participant route; complete participant routes report concrete missing participant-service providers.
