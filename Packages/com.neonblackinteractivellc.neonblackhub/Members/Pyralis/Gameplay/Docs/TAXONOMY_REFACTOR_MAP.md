...
| `Core/Actions` | `Core/Types/Actions` | Core | `NeonBlack.Gameplay.Core` | Core keeps shared action command/context/queue contracts, value language, and the small in-memory queue implementation. Feature modules provide resolvers. | Done |
...
| `Features/Composition` | removed; capability pieces moved to owning modules/Data seams | Module/Data | owning module assemblies | Actor composition was a training-wheel runtime installer. Optional capabilities now live as direct module-owned components/profiles. | Done |
...
| `Features/Pickups` | `Modules/Interaction/Runtime/Collectibles` | Interaction customization lane | `NeonBlack.Gameplay.Modules.Interaction` | Collectible pickup collection, spawning, and feedback are Interaction-owned customization variants, not a separate top-level module. | Done |
...
| Current Path | Target Path | Owner | Assembly | Reason | Phase |
|---|---|---|---|---|---|
| `NeonBlack.Gameplay.asmdef` | Root glue facade | Glue | `NeonBlack.Gameplay` | Root gameplay now references only assemblies needed by glue/session composition; optional modules stay independently compiled unless glue actually wires them. Keep trimming this facade as glue boundaries move into narrower assemblies. | E |
| removed actor-composition assembly | removed | Module/Data | owning module assemblies | Actor composition assembly was deleted after capability contracts moved to direct module-owned homes. | E |
