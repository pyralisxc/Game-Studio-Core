...
#pragma warning restore CS0618
            ...
            return new ResolvedAuthoringContract(
                ...
                proofTargetId: NormalizeProofTargetId(attr),
                proofGuidance: attr.Proof,
                sourceType: type,
                axioms: attr.Axioms,
                ...
                surface: NormalizeSurface(type, attr),
        ...
        private static string NormalizeProofTargetId(AuthoringContractAttribute attr)
        {
            return !string.IsNullOrWhiteSpace(attr.ProofTargetId) ? attr.ProofTargetId : string.Empty;
        }
        ...
        private static AuthoringContractSurface NormalizeSurface(Type type, AuthoringContractAttribute attr)
        {
            if (attr.Surface != AuthoringContractSurface.Auto)
                return attr.Surface;

            if (attr.RoleTags != null)
            {
                for (int i = 0; i < attr.RoleTags.Length; i++)
                {
                    if (string.Equals(attr.RoleTags[i], AuthoringContractRoleTags.IntentRouteEssential, StringComparison.Ordinal))
                        return AuthoringContractSurface.RouteEssential;
                }
            }

            if (type != null && type.IsInterface)
                return AuthoringContractSurface.VocabularyOnly;

            return AuthoringContractSurface.GameplayIngredient;
        }
        ...
        private static ResolvedAuthoringContract MergeContracts(ResolvedAuthoringContract current, ResolvedAuthoringContract incoming)
        {
            return new ResolvedAuthoringContract(
                ...
                proofTargetId: FirstNonEmpty(current.ProofTargetId, incoming.ProofTargetId),
                proofGuidance: FirstNonEmpty(current.ProofGuidance, incoming.ProofGuidance),
                ...
                surface: current.Surface != AuthoringContractSurface.Auto ? current.Surface : incoming.Surface,
