...
using NeonBlack.Gameplay.Glue.Ownership;
...
namespace NeonBlack.Gameplay.Glue.Lifetime
{
    ...
    public class GameplayLifetimeScope : LifetimeScope
    {
        ...
        private RuntimeFeatureServicePolicy _featureServicePolicy;
            ...
            ParticipantInputRouter participantInputRouterOverride = null)
        {
            ...
            _featureServicePolicy = RuntimeFeatureServicePolicy.ResolveWithLoadedSceneEvidence(sessionDefinition);
            _isConfigured = true;
        }
        ...
        {
            RegisterRuntimeFlowServices(builder);
        ...
        {
            var timeScaleService = _timeScaleService != null ? _timeScaleService : FindServiceInHierarchy<TimeScaleService>();
            var cameraShake = _cameraShake != null ? _cameraShake : FindServiceInHierarchy<CameraShake>();

            RegisterSceneNavigator(builder);
            RegisterComponent(builder, timeScaleService);
        ...
        {
            FeatureServiceInstaller.RegisterFeatureServices(
                builder,
            ...
            where T : Component
        {
            ...
            if (existing != null)
            {
                ...
                {
                    Debug.LogError($"[GameplayLifetimeScope] Core service `{serviceName}` is authored as `{existing.GetType().Name}`, but this route expects `{preferredDisplayName}`. Replace the authored component or assign the correct Bootstrap override before Play Mode.", this);
                    return null;
                }
            ...
            Debug.LogError($"[GameplayLifetimeScope] Critical core service `{serviceName}` is missing from the scene. Add an authored child GameObject under the Gameplay Root with `{typeof(T).Name}`, or assign the Bootstrap override field before Play Mode.", this);
            return null;
        }
