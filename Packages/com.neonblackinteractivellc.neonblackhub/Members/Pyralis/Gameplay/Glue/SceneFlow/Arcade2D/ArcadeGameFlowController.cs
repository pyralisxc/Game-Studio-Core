...
[AuthoringContract(
        ...
        CapabilityPath = "Core Setup/Session/Arcade Game Flow Controller",
        ...
        RequiredFields = new[] { nameof(scoreManager), nameof(hazardSpawner), nameof(pickupSpawner), nameof(difficultyController) },
...
[AddComponentMenu("NeonBlack/Gameplay/Glue/SceneFlow/Arcade2D/Arcade Game Flow Controller")]
[DefaultExecutionOrder(-20)]
public partial class ArcadeGameFlowController : MonoBehaviour,
    ...
    IHazardOutcomeSink
{
    ...
    [SerializeField, Tooltip("HazardDifficultyController for this scene.")]
    private HazardDifficultyController difficultyController;
    ...
    private void Awake()
    {
        ...
        difficultyController ??= GetComponent<HazardDifficultyController>();
    ...
    public void StartGame()
    {
        ...
        {
            Debug.LogError("[ArcadeGameFlowController] scoreManager is not assigned in the Inspector.", this);
            return;
        }

        if (difficultyController == null)
        {
            Debug.LogError("[ArcadeGameFlowController] difficultyController is not assigned in the Inspector.", this);
            return;
        }

        if (pickupSpawner == null)
        {
            Debug.LogError("[ArcadeGameFlowController] pickupSpawner is not assigned in the Inspector.", this);
            return;
        }

        if (hazardSpawner == null)
        {
            Debug.LogError("[ArcadeGameFlowController] hazardSpawner is not assigned in the Inspector.", this);
            return;
        }
    ...
    public void PlayerDied(Motor2D deadPlayer)
    {
        ...
        difficultyController?.StopDifficulty();
