...
{
    /// <summary>
    /// Shared validation for the NGO-backed gameplay runtime lane.
    /// </summary>
    [AuthoringContract(
        Category = "Networking",
        Surface = AuthoringSurface.RequiredSetup,
        Summary = "Shared validation for the NGO-backed gameplay runtime lane.",
        RequiredComponentNames = new[] { nameof(NetworkManager), nameof(UnityTransport) },
        Tags = new[] { "capability:Networking" },
        Selectable = false
    )]
    public static class NetworkSetupValidator
    {
        public static List<string> GetIssues(SessionDefinition sessionDefinition, NetworkManager networkManager)
        {
            List<RuntimeValidationIssue> validationIssues = GetValidationIssues(sessionDefinition, networkManager);
        ...
        public static List<RuntimeValidationIssue> GetValidationIssues(SessionDefinition sessionDefinition, NetworkManager networkManager)
        {
            ...
            {
                issues.Add(RuntimeValidationIssue.Required(
                    "NetworkManager uses a non-UnityTransport transport. MVP networking is validated against UnityTransport.",
                    targetLabel: nameof(NetworkManager),
        ...
        {
            List<RuntimeValidationIssue> issues = GetValidationIssues(sessionDefinition, networkManager);
            for (int i = 0; i < issues.Count; i++)
            {
                if (issues[i] != null && issues[i].Severity == RuntimeValidationSeverity.Required)
                    return false;
            }
        ...
        private static void AppendParticipantPawnIssues(SessionDefinition sessionDefinition, NetworkManager networkManager, List<RuntimeValidationIssue> issues)
        {
            ...
            for (int i = 0; i < sessionDefinition.defaultParticipants.Length; i++)
            {
                ...
                {
                    issues.Add(RuntimeValidationIssue.Recommended(
