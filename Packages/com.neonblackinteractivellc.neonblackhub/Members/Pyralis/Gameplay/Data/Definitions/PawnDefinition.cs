...
namespace NeonBlack.Gameplay.Data.Definitions
{
    ...
    [AuthoringContract(
        ...
        RequiredFields = new[] { nameof(pawnPrefab) },
        PrerequisiteStableIds = new[] { "participant.default", "input.profile", "playfield.profile", "camera.rig.profile" },
        RouteStage = "Pawn Asset",
        RouteOrder = 70,
        SetupDomain = "Pawn",
        ProofTarget = "PawnDefinition supplies the prefab and profiles for the participant-owned pawn.",
        NativeActionKind = AuthoringActionKind.CreateAsset,
        SetupSteps = new[] { "Assign Pawn Prefab with PawnRoot and lane-specific pawn components.", "Assign movement, combat, traversal, presentation, and animation profiles only for the capabilities this pawn should support." },
