...
namespace NeonBlack.Gameplay.Modules.Traversal.Editor
{
    ...
    public sealed class Pawn3DTraversalComponentEditor : UnityEditor.Editor
    {
        public override void OnInspectorGUI()
        {
            ...
            PyralisInspectorHandoff.DrawAuthoringButton("Pawn 3D Traversal Component", null);

            DrawDefaultInspector();
            PyralisInspectorValidation.DrawValidationMessages(GetMessages(serializedObject, (Pawn3DTraversalComponent)target), "Pawn3DTraversalComponent is ready for direct 3D pawn traversal setup.");
            serializedObject.ApplyModifiedProperties();
        }

        private static List<PyralisInspectorValidationIssue> GetMessages(SerializedObject serializedObject, Pawn3DTraversalComponent runtime)
        {
            ...
            if (profile != null && profile.objectReferenceValue == null)
                messages.Add(PyralisInspectorValidationIssue.Required("Assign a PawnTraversalProfile so this direct pawn component can apply traversal tuning."));
