...
namespace NeonBlack.Gameplay.Modules.Traversal.Editor
{
    ...
    public sealed class TopDownHopProfileEditor : UnityEditor.Editor
    {
        public override void OnInspectorGUI()
        {
            ...
            InspectorValidation.DrawValidationIssues(GetIssues((TopDownHopProfile)target), "Top-down hop profile is ready for direct TopDownHopComponent assignment.");
    ...
    public sealed class TopDownHopComponentEditor : UnityEditor.Editor
    {
        public override void OnInspectorGUI()
        {
            ...
            DrawDefaultInspector();
            InspectorValidation.DrawValidationMessages(GetMessages(serializedObject), "Top-down hop runtime is ready for direct pawn component setup.");
            serializedObject.ApplyModifiedProperties();
        }

        private static List<InspectorValidationIssue> GetMessages(SerializedObject serializedObject)
        {
            ...
            if (profile != null && profile.objectReferenceValue == null)
                messages.Add(InspectorValidationIssue.Required("Assign a TopDownHopProfile so this direct pawn component can run the hop action."));

            if (visual != null && visual.objectReferenceValue == null)
                messages.Add(InspectorValidationIssue.Optional("Visual Transform is empty. Runtime will lift a child SpriteRenderer or Animator when possible. Assign this field when the pawn art lives under a specific child."));
