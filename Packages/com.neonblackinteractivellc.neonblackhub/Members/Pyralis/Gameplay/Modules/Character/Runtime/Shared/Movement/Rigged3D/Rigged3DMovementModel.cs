...
namespace NeonBlack.Gameplay.Modules.Character
{
    ...
    /// No MonoBehaviour dependency; instantiate with <c>new Rigged3DMovementModel()</c>.
    ///
    /// <para>
    ...
    ///
    /// <code>
    ///   var model = new Rigged3DMovementModel()
    ///   {
    ...
    /// </code>
    /// </summary>
    public sealed class Rigged3DMovementModel
    {
        private readonly MovementState _state = new MovementState();
        private readonly PawnLocomotionStateMachine _locomotionStateMachine = new PawnLocomotionStateMachine();
        ...
        public PawnLocomotionState LocomotionState => _locomotionStateMachine.CurrentState;
        ...
        public Vector3 Tick(MovementInput input, MovementPhysicsFrame physics, float deltaTime)
        {
            ...
            // Absorb last frame's physics results.
            ...
            if (input.JumpPressed)
            {
                ...
                if (_config.AllowJump && !topDownNoJump && !_state.IsCrouching)
                {
            ...
            if (!_state.IsClimbing && !_state.IsHanging && !_state.IsDodging && !_state.IsPowerSliding)
            {
                ...
                Vector3 planarMove = input.MoveWorld.sqrMagnitude > 0f
                    ? input.MoveWorld
                    : new Vector3(input.Move.x, 0f, _config.MovementMode != MovementMode.TwoD ? input.Move.y * _config.DepthSpeedMultiplier : 0f);
                if (planarMove.sqrMagnitude > 1f)
                    planarMove.Normalize();

                float targetX = planarMove.x * targetSpeed;
                float targetZ = _config.MovementMode != MovementMode.TwoD
                    ? planarMove.z * targetSpeed
                    : 0f;
            ...
            // Facing is screen-relative via camera right.
            UpdateFacing(new Vector3(_state.VelocityX, 0f, _state.VelocityZ), input.CameraRight);

            // Cache pre-land velocity for next frame's squash check.
        ...
        // Action triggers called by MonoBehaviour on input events.
        ...
        {
            if (!PawnLocomotionStateMachine.CanStartDodge(_state, _config.AllowDodge))
                return false;
        ...
        {
            if (!PawnLocomotionStateMachine.CanStartPowerSlide(_state, _config.AllowPowerSlide))
                return false;
        ...
        public void SetCrouching(bool crouching) => _state.IsCrouching = _config.AllowCrouch && crouching;
        ...
        /// VelocityY is not reset here; the MB sets it as needed (e.g. -1f to nudge clear of the ledge).
        /// </summary>
        public void NotifyHangEnd()
        {
            _state.IsHanging = false;
            UpdateLocomotionState();
        }

        /// <summary>Directly set VelocityY; used by the MB after hang drop to nudge the character clear.</summary>
        ...
        {
            _locomotionStateMachine.ProjectFrom(_state);
        }
