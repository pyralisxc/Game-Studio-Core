...
///
/// SETUP:
///   1. Attach to a persistent GameObject (ArcadeGameFlowController or DontDestroyOnLoad root).
...
public class GameplaySettingsService : MonoBehaviour, IGameplaySettingsApplier, IInputSettingsRegistrar, IRuntimeValidationProvider
{
    public IEnumerable<RuntimeValidationIssue> GetRuntimeValidationIssues()
    {
        if (settingsProfile == null)
            yield return RuntimeValidationIssue.Required("Settings Profile is required for default volume and deadzone values.");
        
        if (Mixer == null)
            yield return RuntimeValidationIssue.Required("No AudioMixer found. Assign one in the Settings Profile or use the Mixer Override field.");
    }
    [Header("Profile")]
