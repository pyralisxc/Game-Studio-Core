...
namespace NeonBlack.Gameplay.Modules.Combat
{
    ...
    public class HealthComponent : GameplayTickBehaviour, IActorHealthModifierReceiver, IActorHealthState, IActorDamageImmunityController, IRuntimeValidationProvider
    {
        ...
        {
            if (maxHealth <= 0f)
                yield return RuntimeValidationIssue.Required("Max Health must be greater than zero.");

            if (destroyOnDeath && deathDestroyDelay < 0f)
                yield return RuntimeValidationIssue.Required("Death Destroy Delay cannot be negative.");

            if (regenEnabled && regenRate <= 0f)
                yield return RuntimeValidationIssue.Required("Regen is enabled but Regen Rate is <= 0.");
        }
    // Inspector
    [Header("Stats")]
