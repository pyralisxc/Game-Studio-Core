...
namespace NeonBlack.Gameplay.Modules.Combat
{
    ...
    [AuthoringContract(
        ...
        { 
            "NeonBlack.Gameplay.Core.Contracts.IActorMovementModifierReceiver",
    ...
    public class ActorStatusEffectComponent : MonoBehaviour, IActorStatusEffectReceiver, IDamageModifier
{
            ...
            for (int i = _activeEffects.Count - 1; i >= 0; i--)
            {
                    ...
                    || effect.Definition.effectKind == StatusEffectKind.Burn)
                {
                    ...
                    if (effect.TickTimer <= 0f)
                    {
                        ...
                        _health?.TakeDamage(GetEffectMagnitude(effect), transform.position, effect.Source);
                    }
                }
        ...
        private void Initialize()
        {
            ...
            _movementReceiver = GetComponent<IActorMovementModifierReceiver>();
            _combatReceiver = GetComponent<IActorCombatModifierReceiver>();
            _healthModifierReceiver = GetComponent<IActorHealthModifierReceiver>();
            ...
            if (statusProfile != null && statusProfile.startingEffects != null)
            {
                for (int i = 0; i < statusProfile.startingEffects.Length; i++)
                    ApplyStatusEffect(statusProfile.startingEffects[i], gameObject);
            }
        }
        ...
        public void ApplyStatusEffect(StatusEffectDefinition effectDefinition, GameObject source = null)
        {
            ...
            _feedbackPublisher?.PublishStatusApplied(effectDefinition.displayName, effectDefinition.magnitude, source);
            RecomputeAggregates();
        }
        ...
        private void TriggerApplySignal(StatusEffectDefinition definition)
        {
            ...
            if (definition.applySignal == ActorAnimationSignal.Custom && !string.IsNullOrWhiteSpace(definition.customAnimationKey))
                _animation?.TriggerCustom(definition.customAnimationKey);
            else
                _animation?.TriggerSignal(definition.applySignal);
        }
