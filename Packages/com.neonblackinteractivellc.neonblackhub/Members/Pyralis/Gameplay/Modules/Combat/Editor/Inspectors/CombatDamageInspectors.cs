...
namespace NeonBlack.Gameplay.Modules.Combat.Editor
{
    ...
    public sealed class HitBox2DEditor : UnityEditor.Editor
    {
        public override void OnInspectorGUI()
        {
            ...
            InspectorValidation.DrawValidationMessages(GetHitBox2DMessages(serializedObject, (HitBox2D)target), "HitBox2D is ready for timed 2D attack contact.");
            serializedObject.ApplyModifiedProperties();
        }

        private static List<InspectorValidationIssue> GetHitBox2DMessages(SerializedObject serializedObject, HitBox2D hitBox)
        {
            ...
            if (collider == null)
                messages.Add(InspectorValidationIssue.Required("Collider2D is required for 2D hit detection."));
            ...
            {
                messages.Add(InspectorValidationIssue.Optional("Owner is empty and no parent HealthComponent was found. Assign Owner so self/friendly hits can be filtered."));
            }
            ...
            {
                messages.Add(InspectorValidationIssue.Required("Base Damage must be greater than zero when Weapon is empty."));
            }
            ...
            if (freezeFrameDuration != null && freezeFrameDuration.floatValue > 0f)
            {
                if (hitPauseSink == null || hitPauseSink.objectReferenceValue == null)
                    messages.Add(InspectorValidationIssue.Recommended("Hit Pause Sink is empty. Assign TimeScaleService or another IHitPauseSink when Freeze Frame Duration is greater than zero."));
                else if (!ImplementsInterface(hitPauseSink.objectReferenceValue, "NeonBlack.Gameplay.Core.Contracts.IHitPauseSink"))
                    messages.Add(InspectorValidationIssue.Required("Hit Pause Sink must reference a component that implements IHitPauseSink."));
            }
            return messages;
        }
    }
    ...
    public sealed class ProjectileEditor : UnityEditor.Editor
    {
        public override void OnInspectorGUI()
        {
            ...
            DrawDefaultInspector();
            InspectorValidation.DrawValidationMessages(GetProjectileMessages(serializedObject, (Projectile)target), "Projectile is ready for ranged combat launch.");
            serializedObject.ApplyModifiedProperties();
        }

        private static List<InspectorValidationIssue> GetProjectileMessages(SerializedObject serializedObject, Projectile projectile)
        {
            List<InspectorValidationIssue> messages = new List<InspectorValidationIssue>();
            GameObject root = projectile != null ? projectile.gameObject : null;

            if (root != null && root.GetComponent<Rigidbody>() == null)
                messages.Add(InspectorValidationIssue.Required("Rigidbody is required on the projectile root."));
            ...
            if (root != null)
            {
                ...
                {
                    messages.Add(InspectorValidationIssue.Required("Projectile needs at least one trigger Collider on the prefab root."));
                }
                else
                {
                    ...
                    if (!hasTrigger)
                        messages.Add(InspectorValidationIssue.Required("Projectile needs at least one trigger Collider so OnTriggerEnter can fire."));
                }
            }

            SerializedProperty lifetime = serializedObject.FindProperty("lifetime");
            if (lifetime != null && lifetime.floatValue <= 0f)
                messages.Add(InspectorValidationIssue.Required("Lifetime must be greater than zero."));
            ...
            {
                messages.Add(InspectorValidationIssue.Required("Camera Shake Sink must reference a component that implements ICameraShakeSink."));
            }
    ...
    public sealed class Projectile2DEditor : UnityEditor.Editor
    {
        public override void OnInspectorGUI()
        {
            ...
            DrawDefaultInspector();
            InspectorValidation.DrawValidationMessages(GetProjectile2DMessages((Projectile2D)target), "Projectile2D is ready for 2D projectile prefab delivery.");
            serializedObject.ApplyModifiedProperties();
        }

        private static List<InspectorValidationIssue> GetProjectile2DMessages(Projectile2D projectile)
        {
            List<InspectorValidationIssue> messages = new List<InspectorValidationIssue>();
            GameObject root = projectile != null ? projectile.gameObject : null;

            if (root != null && root.GetComponent<Rigidbody2D>() == null)
                messages.Add(InspectorValidationIssue.Required("Rigidbody2D is required on the projectile root."));

            Collider2D collider = root != null ? root.GetComponent<Collider2D>() : null;
            if (collider == null)
                messages.Add(InspectorValidationIssue.Required("Collider2D is required for 2D projectile trigger detection."));
            else if (!collider.isTrigger)
                messages.Add(InspectorValidationIssue.Required("Collider2D should be set to Is Trigger."));
    ...
    public sealed class KnockbackReceiverEditor : UnityEditor.Editor
    {
        public override void OnInspectorGUI()
        {
            ...
            DrawDefaultInspector();
            InspectorValidation.DrawValidationMessages(GetKnockbackMessages(serializedObject, (KnockbackReceiver)target), "KnockbackReceiver is ready for 3D actor knockback.");
            serializedObject.ApplyModifiedProperties();
        }

        private static List<InspectorValidationIssue> GetKnockbackMessages(SerializedObject serializedObject, KnockbackReceiver receiver)
        {
            List<InspectorValidationIssue> messages = new List<InspectorValidationIssue>();
            GameObject root = receiver != null ? receiver.gameObject : null;

            if (root != null && root.GetComponent<CharacterController>() == null)
                messages.Add(InspectorValidationIssue.Required("CharacterController is required for 3D knockback."));
            ...
            if (decayRate != null && decayRate.floatValue <= 0f)
                messages.Add(InspectorValidationIssue.Required("Decay Rate must be greater than zero."));
    ...
    public sealed class HitFlashEditor : UnityEditor.Editor
    {
        public override void OnInspectorGUI()
        {
            ...
            DrawDefaultInspector();
            InspectorValidation.DrawValidationMessages(GetHitFlashMessages(serializedObject, (HitFlash)target), "HitFlash is ready for 2D damage feedback.");
            serializedObject.ApplyModifiedProperties();
        }

        private static List<InspectorValidationIssue> GetHitFlashMessages(SerializedObject serializedObject, HitFlash hitFlash)
        {
            List<InspectorValidationIssue> messages = new List<InspectorValidationIssue>();
            GameObject root = hitFlash != null ? hitFlash.gameObject : null;

            if (root != null && root.GetComponent<SpriteRenderer>() == null)
                messages.Add(InspectorValidationIssue.Required("SpriteRenderer is required on the same GameObject."));
            ...
            if (duration != null && duration.floatValue <= 0f)
                messages.Add(InspectorValidationIssue.Required("Flash Duration must be greater than zero."));
    ...
    {
        public static void RequirePositive(SerializedObject serializedObject, List<InspectorValidationIssue> messages, string propertyName, string displayName)
        {
        ...
        public static void RequireNonNegative(SerializedObject serializedObject, List<InspectorValidationIssue> messages, string propertyName, string displayName)
        {
            SerializedProperty property = serializedObject.FindProperty(propertyName);
            if (property != null && property.floatValue < 0f)
