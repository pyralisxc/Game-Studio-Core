...
namespace NeonBlack.Gameplay.Features.Feedback.UI
{
    ...
    public class ParticipantFeedbackHudPresenter : ParticipantHudTargetBinding, IRuntimeValidationProvider
    {
        ...
        private void HandleFeedbackMessage(ParticipantFeedbackMessage message)
        {
            ...
            switch (message.Kind)
            {
                case ParticipantFeedbackKind.Combo:
                    ...
                    ShowDirectLabel(comboLabel, $"Combo {message.IntValue}", ref _comboTimer, comboDisplayTime);
                    break;
                case ParticipantFeedbackKind.Status:
                    ShowPanels(statusPanels, message.TextValue, statusDisplayTime);
                    ShowDirectLabel(statusLabel, message.TextValue, ref _statusTimer, statusDisplayTime);
                    break;
                case ParticipantFeedbackKind.Score:
                    string scoreText = $"+{message.IntValue}";
                    ShowPanels(scorePanels, scoreText, scoreDisplayTime);
                    ShowDirectLabel(scorePopupLabel, scoreText, ref _scoreTimer, scoreDisplayTime);
                    break;
                case ParticipantFeedbackKind.CombatAlert:
                    ...
                    ShowDirectLabel(combatAlertLabel, alertText, ref _combatAlertTimer, combatAlertDisplayTime);
                    break;
            }
        }
        ...
        public IEnumerable<string> GetRuntimeValidationIssues()
        {
            ...
            bool hasDirectLabelSurface = comboLabel != null
                || statusLabel != null
        ...
        private static void ShowDirectLabel(TextMeshProUGUI label, string text, ref float timer, float duration)
        {
            if (label == null)
                return;
