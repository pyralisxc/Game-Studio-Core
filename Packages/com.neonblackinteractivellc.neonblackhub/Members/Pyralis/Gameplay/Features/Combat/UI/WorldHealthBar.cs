...
namespace NeonBlack.Gameplay.Features.Combat
{
    ...
    ///
    /// Features:
    ///  Ghost / drain bar - optional amber bar lingers then slowly drains on damage
    ///  Smooth animated HP fill with configurable speed
    ///  Always-green fill - width shrinks with HP, revealing the lost-HP colour behind it
    ///  Border layer (configurable thickness)
    ///  Segment dividers splitting the bar into equal sections
    ///  Scale-punch animation when taking damage
    ///  Low-HP pulse flash at configurable threshold
    ///  Optional floating character name label above the bar
    ///  Optional live HP number display inside the bar
    ///  Auto-spawns damage/heal numbers via DamageNumberSpawner (if present)
    ///  Auto-hide with smooth fade; or always-visible mode
    ///
    /// Setup: Add this component to any root GameObject that also has a HealthComponent.
    ///  Everything is built at runtime - no prefab or Canvas needed in the scene.
    /// </summary>
    ...
    public class WorldHealthBar : MonoBehaviour
    {
        ...
        [Tooltip("Fill colour at the mid-point - used only when Fill Gradient is enabled.")]
        ...
        [Tooltip("Blend the fill colour from Fill Colour to Mid HP Colour to Low HP Colour as HP falls. Disable to keep a constant fill colour.")]
        ...
        [Tooltip("Optional damage-number service. Assign DamageNumberSpawner or another IDamageNumberSink when damage/heal numbers are enabled.")]
        [SerializeField] private MonoBehaviour damageNumberSink;

        // Camera
        [Header("Camera")]
        [Tooltip("Camera used to face the world-space bar toward the viewer. Leave empty only when another system sets it at runtime.")]
        ...
        private void BuildCanvas()
        {
            ...
            // Lost HP region - full-width coloured layer visible wherever the fill doesn't cover
            Image emptyImg = MakeRect(root, "EmptyHP", emptyHpColor, CW, ch);
            // Ghost bar (amber - stays put on damage, drains after delay)
            if (showGhostBar)
                _ghost = MakeFill(root, "Ghost", ghostColor, CW, ch);

            // HP fill (green - snaps/animates to current HP)
            ...
            // Rounded corners - generate once and apply to every bar layer
            if (cornerRadius > 0f)
            {
