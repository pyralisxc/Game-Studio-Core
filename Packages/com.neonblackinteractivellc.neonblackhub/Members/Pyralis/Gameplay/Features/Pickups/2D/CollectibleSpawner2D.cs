...
public class CollectibleSpawner2D : MonoBehaviour, IPickupSpawnSurface, IPickupBurstSpawnSurface
{
    ...
    {
        if (!TryGetCameraBounds(_spawnMargin + _crumbSizeRadius, out CameraBounds2D bounds))
        {
        ...
        do
        {
            position = new Vector2(
                Random.Range(camPos.x - bounds.HalfWidth, camPos.x + bounds.HalfWidth),
                Random.Range(camPos.y - bounds.HalfHeight, camPos.y + bounds.HalfHeight));
            attempts++;
        }
